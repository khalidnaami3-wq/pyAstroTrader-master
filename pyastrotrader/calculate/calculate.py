...
def calculate_planets(input_data, intermediate, output, config):
    ...
    for i in range(NUM_PLANETS):
        ...
        coordinates = ret_flag
        if len(coordinates) == 2 and isinstance(coordinates[0], tuple):
            coordinates = coordinates[0]

        for x in range(len(config['zodiac'])):
            deg_low = float(x * 30)
            deg_high = float((x + 1) * 30)
            if coordinates[0] >= deg_low:
                if coordinates[0] <= deg_high:
                    output['planets']['planets_sign_name'][
                        i] = config['planet_name_short'][i] + ":" + config[
                            'zodiac'][x] + ":" + str(x)
                    output['planets']['planets_degree'][
                        i] = coordinates[0] - deg_low
                    output['planets']['planets_degree_ut'][i] = coordinates[0]
                    if coordinates[3] < 0:
